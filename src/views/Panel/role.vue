<template>
    <DefaultLayout>
        <SubNav3 />
        <BreadcrumbDefault pageTitle="Role" />
        ...
        <div class="font-serif ml-56" ref="scrollableDiv">
            <div class="mb-2 ml-32">
                <div v-if="showcreate" class="shadow-md shadow-gray-400 w-72 h-32 ml-20">
                    <div class="pt-5 p-2">
                        <label class=""> Enter Role:</label>
                        <input type="text" name="payment_name" v-model="name" class="bg-gray-200 rounded-md">
                    </div>

                    <button @click="addRole" class="btn bg-teal-700 py-1 px-4 rounded-md text-white">Add</button>
                    <button @click="discardCreate"
                        class="btn bg-gray-400 rounded-md py-1 px-4 ml-2 text-white">cancle</button>
                </div>
                <button @click="createRole" class="btn bg-cyan-800 text-white py-2 px-8 -ml-80 rounded-md">
                    Add Role
                </button>
            </div>
            ...
            <div v-else>
                ...
                <div v-for="role in roles" :key="role.id"
                    ...
                    <div class="col-span-1 hidden items-center sm:flex">
                        <p class="text-sm font-medium text-black dark:text-white">{{role.name}}</p>
                    </div>
                    ...
                    <div class="col-span-1 flex items-center ml-96">
                        <div class="py-5 px-4 ml-48">
                            <div class="flex items-center space-x-3.5">
                              ...
                              <ficon :icon="['fas', 'pen-to-square']" @click="startEdit(role)" />
                             <ficon :icon="['fas', 'trash']" @click="deleteRole(role.id)"/>
...
export default {
    ...
    methods: {
        ...
        editRole() {
            ...
            const updatedMethod = { name: this.name };

            axios.put(`http://127.0.0.1:8000/api/role/${this.roleID}`, updatedMethod)
                .then(res => {
        ...
        addRole() {
            this.model.role.name = this.name;
            axios.post('http://127.0.0.1:8000/api/role', this.model.role)
                .then(res => {
        ...
        deleteRole(roleID) {
            if (confirm('Are you sure you want to delete this payment method?')) {
                axios.delete(`http://127.0.0.1:8000/api/role/${roleID}`)
                    .then(res => {
                        console.log('Role deleted successfully', res.data);
                        this.fetchRole();
                    })
                    .catch(error => {
                        console.error('Error deleting Role method', error.response.data);
                    });
            }
        }
    }
