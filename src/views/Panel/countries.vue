        ...
        <div class="font-serif ml-56" ref="scrollableDiv">
            <div class="mb-2 ml-32">
                <div v-if="showcreate" class="shadow-md shadow-gray-400 w-72 h-32 ml-20">
                    <div class="pt-5 p-2">
                        <label class=""> Enter country name:</label>
                        <input type="text" name="name" v-model="name" class="bg-gray-200 rounded-md">
                    </div>
                    <div class="pt-5 p-2">
                        <label class=""> Enter country code:</label>
                        <input type="text" name="name" v-model="code" class="bg-gray-200 rounded-md">
                    </div>

                    <button @click="addCountry" class="btn bg-teal-700 py-1 px-4 rounded-md text-white">Add</button>
            ...
            <div v-else>
                ...
                <div v-for="country in countries" :key="country.id"
                    ...
                    <div class="col-span-1 hidden items-center sm:flex">
                        <p class="text-sm font-medium text-black dark:text-white">{{country.name}}</p>
                    </div>
                    <div class="col-span-1 flex items-center ">
                        <p class="text-sm font-medium text-black dark:text-white ml-80">{{country.code}}</p>
                    </div>
                    ...
                    <div class="col-span-1 flex items-center ml-96">
                        <div class="py-5 px-4 ml-48">
                            <div class="flex items-center space-x-3.5">
                              ...
                              <ficon :icon="['fas', 'pen-to-square']" @click="startEdit(country)" />
                             <ficon :icon="['fas', 'trash']" @click="deleteCountry(country.id)"/>
...
export default {
    ...
    methods: {
        ...
        editCountry() {
            ...
            axios.put(`http://127.0.0.1:8000/api/country/${this.countryID}`, updatedMethod)
                .then(res => {
                    console.log('Country updated successfully', res.data);
                    this.name = '';
                    this.code = ''
                    this.showedit = false;
                    this.fetchCountry();
        ...
        addCountry() {
            this.model.country.name = this.name;
            this.model.country.code = this.code;
            axios.post('http://127.0.0.1:8000/api/country', this.model.country)
                .then(res => {
                    console.log('Country created successfully', res.data);
                    this.name = '';
                    this.code = ''
                    this.fetchCountry();
                })
       ...
       this.isLoading = false
        ...
        deleteCountry(countryID) {
            if (confirm('Are you sure you want to delete this payment method?')) {
                axios.delete(`http://127.0.0.1:8000/api/country/${countryID}`)
                    .then(res => {
                        console.log('country deleted successfully', res.data);
                        this.fetchCountry();
                    })
                    .catch(error => {
