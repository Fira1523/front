<template>
    ...
    <div class="font-serif ml-56" ref="scrollableDiv">
      <h1 class="-ml-56 mb-4 text-2xl text-black font-blod">Service Fee</h1>
            <div class="mb-2 ml-32">
                <div v-if="showcreate" class="shadow-md shadow-gray-400 w-72 h-32 ml-20">
                                 <div>
          <label for="" class="font-serif  text-md">Percent:</label><br>
          ...
          <input type="number" name="certificate" class="border-2 mt-3 lg:w-60 w-48 mb-3 border-gray-200 focus:outline-none h-8 rounded-md lg:mr-1 bg-white" required v-model="code">
                ...
                <button @click="createPaymethod" class="btn bg-cyan-800 text-white py-2 px-8 -ml-80 rounded-md mb-3">
                    Add ServiceFee
                </button>
            </div>
            ...
            <div v-else>
                ...
                <div 
                    ...
                    <div class="col-span-1 hidden items-center sm:flex">
                        <p class="text-sm font-medium text-black dark:text-white">{{percent}}</p>
                    </div>
                    <div class="col-span-1 flex items-center ml-5">
                        <p class="text-sm font-medium text-black dark:text-white ml-80">{{status}}</p>
                    </div>
    ...
    methods:{
      fetchServiceFees(){
        axios.get('http://127.0.0.1:8000/api/service').then((res)=>{
          this.seviceFees = res.data.data
          this.percent = res.data.data[0].percent
          this.status = res.data.data[0].state
          console.log('service', this.percent)
        })
      }
    }
}
