<template>
    <div class="shadow-md mt-20 ml-96 mr-96 h-96 pt-12 pb-96 pl-10 bg-white rounded-full">
        ...
        <p class="mt-10" v-if="showText">{{ text }}</p>
...
export default {
    ...
    mounted() {
        ...
        this.model.chats.sender_id = localStorage.getItem('user_id'); 
        this.model.chats.receiver_id = localStorage.getItem('receiver_id');
    ...
    methods: {
        handleChat() {
            ...
            localStorage.setItem('message',this.messages)
        },
        postChat(){
            
            const sendMessages =  localStorage.getItem('message',this.messages)
                // this.model.chats = this.messages
            
          axios.get('http://127.0.0.1:8000/api/chat', sendMessages).then((res)=>[
